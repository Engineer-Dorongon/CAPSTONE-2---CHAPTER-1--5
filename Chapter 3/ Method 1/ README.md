...
$$R_{Failure\_Center} = \frac{\sum (FP_{Center} + FN_{Center})}{480} \times 100$$You are absolutely right, and I apologize for that omission. Since we added **Level C (Column-by-Column Category Performance)** to your Chapter 4 Results to prove how the camera's wide-angle lens distortion affects the AI, we **must** explicitly declare the formulas for Level C in your **Chapter 3 (Statistical Treatment of Data)**. 

If it is calculated in Chapter 4, the exact formula must exist in Chapter 3 to pass a rigorous engineering panel defense.

Here is the fully completed **Statistical Treatment of Data (SOP 1)** section for your Chapter 3. It now correctly includes **Level A (Overall)**, **Level B (Row-by-Row)**, and the newly added **Level C (Column-by-Column)**.
