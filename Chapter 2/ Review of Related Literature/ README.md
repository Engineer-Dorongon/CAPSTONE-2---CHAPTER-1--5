...
* **Summary:** This foreign book presents the mathematical foundations of 3D computer vision, detailing pinhole camera projection parameters, perspective visual transformations, and coordinate mapping matrices [15]. 
* **Critique and Connection:** Szeliski’s theories provide the baseline equations used to project 2D camera pixels into 3D camera-space coordinates. However, Szeliski’s standard algorithms assume a static, non-moving camera [15]. SENSEY extends this theory by integrating a live IMU sensor to rotate these camera coordinates, ensuring they remain mathematically stable relative to gravity.

### **2. Wiener, Welsh, and Blasch (2010) – *Foundations of Orientation and Mobility***
* **Summary:** This foreign textbook establishes the standards of Orientation and Mobility (O&M) science, defining how standard human dimensions, walking speeds, and chest-height perspectives dictate the physical space required to clear obstacles safely [16].
...
* **Summary:** This local journal study documents the day-to-day physical mobility struggles of disabled employees inside Philippine public school buildings, revealing that classroom aisles are narrow, cluttered, and completely lack structural mobility accommodations [4].
* **Critique and Connection:** This study provides the localized environmental justification for SENSEY's obstacle-avoidance module. It proves that a **visually impaired teacher** navigating Philippine public schools requires a highly sensitive, real-time warning sensor to bypass physical clutter safely [4].
...
* **Summary:** This local alumni study developed a wearable obstacle detector using an Xbox 360 Kinect camera connected to a laptop inside a backpack, applying Canny edge detection to depth-thresholded images to warn users of obstacles in Left, Center, and Right zones at a fixed $1.0\text{m}$ boundary [11].
* **Critique and Connection:** This system coheres with SENSEY’s goal of partitioning the environment into lateral lanes [11]. However, it suffered from a massive $0.8\text{m}$ close-range hardware blind spot, had no camera-tilt compensation, and required a heavy laptop and external power [11]. SENSEY directly modernizes this baseline by using a lightweight, chest-mounted OAK-D Lite depth camera that reduces the close-range blind spot to $0.4\text{m}$.

### **2. Wearable Wrist Haptic Device (Alayon, Corciega, Tolentino, et al., 2020)**
* **Summary:** Published in the IEEE database, this local alumni study under your adviser developed a wearable wristband containing 7 physical solenoids. It used a Kinect depth sensor to detect obstacle angles and translated them into a base-2 binary tapping code to tap the user's wrist to indicate direction [12].
* **Critique and Connection:** While this system achieved a $99.39\%$ user perception rate, the authors concluded that the binary tapping carried a high cognitive load, requiring users to undergo extensive training to translate mathematical tapping into walking directions [12]. 

To eliminate these dynamic physical distortions, SENSEY implements a **3D Spatial Coordinate Euler Rotation Algorithm** using live IMU pitch values:

$$Y_{world} = Y_c \cos(\theta) - Z_c \sin(\theta)$$

$$Z_{world} = Y_c \sin(\theta) + Z_c \cos(\theta)$$

This rotates the depth coordinates in real-time, completely eliminating the "pitch-induced floor detection" false alarms that plagued both Alayon (2020) and Barrinuevo (2017) when the user bent forward [11, 12].

### **3. Smart Cane Assistive Device (Htike et al., 2020)**
* **Summary:** This regional local study developed a standard walking cane integrated with ultrasonic distance sensors to provide beep alerts when approaching low-profile floor hazards [8].
* **Critique and Connection:** While this system successfully detected low-lying hazards, it occupied the user's hand [8]. This is a major limitation for a **visually impaired teacher**, who needs their hands free to hold papers, write on a board, or handle teaching materials. SENSEY resolves this by using a hands-free, chest-mounted design.

### **4. The AIris Wearable Assistant (Brilli et al., 2024)**
* **Summary:** This foreign system reviews *AIris*, a chest-mounted, camera-based edge-AI system utilizing local neural processing to detect obstacles and output spoken verbal feedback to guide blind pedestrians [7].
* **Critique and Connection:** This system serves as your primary foreign system benchmark [7]. However, AIris lacks the parallel local depth sensors of SENSEY, making its steering warnings slower and less responsive in highly dynamic, crowded indoor environments like classrooms [7].

### **5. Fuzzy Decision Wearable Helper (Bouteraa, 2021)**
* **Summary:** This foreign system reviews a wearable depth-based navigation system that utilizes custom fuzzy-logic algorithms to segment camera depth maps into Left, Center, and Right safety lanes to output steering instructions [9].
* **Critique and Connection:** While Bouteraa (2021) validates SENSEY's choice of lateral lane division for finding walkable paths, the system is strictly limited to basic steering instructions [9]. It lacks any student posture or behavior status tracking, rendering it incapable of assisting a teacher in managing a classroom [9].

### **6. Object Detection/Tactile Presentation System (Chen et al., 2023)**
* **Summary:** This foreign system reviews a wearable computer-vision vest that detects obstacles using YOLO and converts their coordinates into a grid of vibration-based haptic patterns on the user's chest [5].
* **Critique and Connection:** The haptic-based vest of Chen et al. (2023) is heavy, physically complex, and has high power consumption, making it physically fatiguing for a teacher [5]. SENSEY improves upon this baseline by using a purely auditory, non-blocking multimodal feedback system (combining spatial audio metronomes with spoken reports) that provides excellent environmental awareness without physical fatigue or complex wiring.

---

## **2.3 Research Gap**

To evaluate how SENSEY contributes to the existing body of knowledge, a comparative feature analysis of the reviewed systems was conducted. The evaluation is structured across eight (8) high-level user features and operational capabilities, directly representing the core requirements of your study's objectives.

The baseline comparison of these six (6) systems and studies is presented in Table 2.1:

### **Table 2.1: Feature-Based Comparative Analysis of Assistive Systems and Related Studies**

| Study / System | Depth-Based Distance Sensing | Chest Tilt Auto-Correction | Floor Hazard Warning (Shoes, Bags, etc.) | Clear Walking Path Finder | Student Posture Tracking (Sitting/Standing) | Classroom Behavior Monitoring | Spoken Surrounding Descriptions (Claude AI) | Hands-Free Auditory Steering |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| ***Precise Obstacles Avoidance System for Visually Impaired People using Xbox 360 Kinect*** (2017) | YES | NO | NO | YES | NO | NO | NO | YES |
| ***Design of Wearable Wrist Haptic Device for Blind Navigation using Microsoft Kinect for Xbox 360*** (2020) | YES | NO | NO | YES | NO | NO | NO | NO |
| ***Smart Cane: Assistive Cane for Visually-impaired People*** (2020) | NO | NO | NO | NO | NO | NO | NO | NO |
| ***AIris: An AI-powered Wearable Assistive Device for the Visually Impaired*** (2024) | YES | NO | NO | NO | NO | NO | YES | YES |
| ***Design and Development of a Wearable Assistive Device Integrating a Fuzzy Decision Support System for Blind and Visually Impaired People*** (2021) | NO | NO | NO | YES | NO | NO | NO | NO |
| ***A Wearable Assistive System for the Visually Impaired Using Object Detection, Distance Measurement and Tactile Presentation*** (2023) | YES | NO | NO | NO | NO | NO | NO | NO |
| **SENSEY (This Study)** | **YES** | **YES** | **YES** | **YES** | **YES** | **YES** | **YES** | **YES** |

---

### **Synthesis of the Research Gap**

As demonstrated in Table 2.1, a comparative analysis of the six reviewed systems reveals distinct feature gaps that limit their practical use for a **visually impaired teacher**:

* **Classroom Monitoring and Auditing Gaps (Objective 1):**  
  None of the six completed systems have the capability to perform **Student Posture Tracking** or **Classroom Behavior Monitoring** [3]. These systems were designed strictly for general pedestrian travel, leaving the visually impaired teacher entirely blind to student presence, active hand-raising, or off-task behaviors [3, 11]. SENSEY is the only device that provides these educational auditing capabilities, allowing the teacher to manage their classroom independently.
* **Ground Terrain and Posture Gaps (Objective 2):**  
  While several systems feature depth sensors and path-finding capabilities (Barrinuevo et al., 2017; Alayon et al., 2020), they completely lack **Chest Tilt Auto-Correction** [11]. When walking or bending down, the camera tilt distorts depth measurements, causing standard systems to mistake the flat floor for a wall [11]. Furthermore, they lack the sensitivity to provide **Floor Hazard Warnings** for small tripping obstacles under 15 cm. SENSEY solves these gaps by integrating an automatic pitch stabilizer to filter out the flat floor and detect low-profile floor hazards (like shoes, backpacks, and cables) down to 7 cm.
* **Auditory Usability and Surrounding Context Gaps (Objective 3):**  
  Many existing devices rely on heavy haptic vests (Chen et al., 2023) or complex wristband tapping patterns (Alayon et al., 2020) that require extensive training, while other devices (like standard canes) occupy the user's hands. Furthermore, standard navigation aids only tell the user where to turn, leaving them blind to the actual layout of their room. SENSEY bridges this gap by being completely **Hands-Free**, utilizing an intuitive spatial audio metronome for steering while providing detailed, **Spoken Surrounding Descriptions** of the room on a single button press [17].
...
https://vertexaisearch.cloud.google.com/grounding-api-redirect/AUZIYQFfJi8GowO1tLJVMxiCEXmfv3_8IplvmriSKrOKBOIUIU98
...
Bouteraa, Y. (2021). Design and development of a wearable assistive device integrating a fuzzy decision support system for blind and visually impaired people. *Micromachines*, *12*(9), 1082. https://doi.org/10.3390/mi12091082
